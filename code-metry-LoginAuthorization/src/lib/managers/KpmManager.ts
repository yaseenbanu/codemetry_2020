...
import { getRootPathForFile, isFileActive, getFileAgeInDays, getFileType, getNowTimes, isEmptyObj } from "../../Util";
...
export class KpmManager {
    ...
    private async _onEventHandler(event : any) {
        ...
        if (!isCharDelete && textChangeLen === 0 && linesAdded === 0 && linesDeleted === 0) {
            return;
        }
        ...
        sourceObj.keystrokes = sourceObj.add - sourceObj.delete;
        sourceObj.lines = currLineCount;
        sourceObj.linesAdded += linesAdded;
        sourceObj.linesRemoved += linesDeleted;
        rootObj.compileTimeErrors = rootObj.compileTimeErrors;
    ...
    private getFileName(event: { fileName: string; document: { fileName: string; }; }) {
        let filename = "";
        if (event.fileName) {
    ...
    private isTrueEventFile(event: { uri: { scheme: string; };
        document: { uri: { scheme: string; }; }; }, filename: string) {
        ...
        if ((scheme !== "file" && scheme !== "untitled") || isLiveshareTmpFile 
            || isInternalFile || !isFileActive(filename)) {
    ...
    private async getStaticEventInfo(event: { fileName: any; languageId: string; getText: () => { (): any; new(): any; 
                    length: number; }; lineCount: number; document: { fileName: any; languageId: string; getText: () => { (): any; new(): any; length: number; }; 
                    lineCount: number; }; }, filename: string) {
    ...
    private addFile(filename: string, nowTimes: { now_in_sec: any; local_now_in_sec: any; }, 
        keystrokeStats: { source: { [x: string]: any; }; }) {
        const timestamp = new Date(Date.now()).toString();

        const fileInfo = new FileChangeInfo();
        fileInfo.start = timestamp;
    ...
    private async createKeystrokeStats(filename: string | number, rootPath: string, 
        nowTimes: { now?: import("moment").Moment; now_in_sec: any; offset_in_sec?: number; 
        local_now_in_sec: any; utcDay?: string; day?: string; localDayTime?: string; }) {
